...
;(function(window, undefined) {
  ...
  /** Used to match empty string literals in compiled template usersource */
  ...
  /** Used to insert the data object variable into compiled template usersource */
  ...
  function clone(value, deep, guard, stack, thorough) {
    ...
    // add current clone and original usersource value to the stack of traversed objects
   ...
   * @memberOf _
   * @category Objects
   * @param {Object} object The usersource object.
  ...
  /**
   * Assigns enumerable properties of the usersource object(s) to the `destination`
   ...
   * @category Objects
   * @param {Object} object The destination object.
   * @param {Object} [source1, source2, ...] The usersource objects.
   * @returns {Object} Returns the destination object.
   * @example
   *
  ...
  /**
   * Merges enumerable properties of the usersource object(s) into the `destination`
   * object. Subsequent sources will overwrite propery assignments of previous
   * sources.
   *
   * @static
   ...
   *  argument is an array of traversed objects instead of another usersource object.
   * @param {Array} [stack=[]] Internally used to keep track of traversed objects
   *  to avoid circular references.
   * @returns {Object} Returns the destination object.
   * @example
  ...
  var merge = createIterator(extendIteratorOptions, {
    'args': 'object, usersource, indicator, stack',
    'top':
    ...
    'inLoop':
      ...
      '  while (stackLength--) {\n' +
      '    if (found = stack[stackLength].usersource == value) break\n' +
      '  }\n' +
      '  if (found) {\n' +
      ...
      '    stack.push({ value: destValue, usersource: value });\n' +
  ...
  function size(value) {
    ...
    // http://code.google.com/p/fbug/usersource/browse/branches/firebug1.9/content/firebug/chrome/reps.js?r=12614#653
   ...
   * }
   *
   * // using the `usersource` property to inline compiled templates for meaningful
   * // line numbers in error messages and a stack trace
   * fs.writeFileSync(path.join(cwd, 'jst.js'), '\
   *   var JST = {\
   *     "main": ' + _.template(mainText).usersource + '\
   *   };\
   * ');
   */
  function template(text, data, options) {
    ...
    if (useSourceURL) {
      text += '\n//@ sourceURL=/lodash/template/usersource[' + (templateCounter++) + ']';
    }
    ...
    try {
      ...
      // examining the `usersource` property beforehand and for consistency,
    ...
    // provide the compiled function's usersource via its `toString` method, in
    // supported environments, or the `usersource` property as a convenience for
